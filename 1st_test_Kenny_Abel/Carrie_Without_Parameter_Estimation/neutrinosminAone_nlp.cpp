...
bool NEUTRINOS_NLP::eval_f(Index n, const Number* x, bool new_x, Number& obj_value)
{
  ...
  for(Index jt=0;jt<Time;jt++) {
    ...
    obj_value += Xdval[1]*pow(-Xdval[0] + Xval[0], 2) + Xdval2[1]*pow(-Xdval2[0] + Xval2[0], 2) + pow(Xval[1] - Xdval[2], 2) + pow(Xval2[1] - Xdval2[2], 2); 
...
bool NEUTRINOS_NLP::eval_grad_f(Index n, const Number* x, bool new_x, Number* grad_f)
{
  ...
  for(Index jt=0;jt<Time;jt++) {
    ...
    grad_f[jt+1*(Time+1)] += (2*Xval[1] - 2*Xdval[2])/(2*Time+1);
    grad_f[(Time+1)*(nU+nY) + 0*Time + jt] += (Xdval2[1]*(-2*Xdval2[0] + 2*Xval2[0]))/(2*Time+1);
    grad_f[(Time+1)*(nU+nY) + 1*Time + jt] += (2*Xval2[1] - 2*Xdval2[2])/(2*Time+1);
...
// Add last element
    ...
    grad_f[Time+1*(Time+1)] += (2*Xval[1] - 2*Xdval[2])/(2*Time+1);
